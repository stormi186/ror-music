class TracksController < ApplicationController
    ...
    @top_tracks = Favorite.joins('join tracks on favorites.track_id = tracks.id').group('track_id').select('id, track_id, name, artist_id').order('COUNT(tracks.id) DESC').limit(10)
