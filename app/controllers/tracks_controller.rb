class TracksController < ApplicationController
    ...
    @top_tracks = Favorite.joins('join tracks on favorites.track_id = tracks.id').select('favorites.id, favorites.user_id, favorites.track_id, favorites.created_at, favorites.updated_at, tracks.name, tracks.artist_id').group('track_id').order('track_id DESC').limit(10)
