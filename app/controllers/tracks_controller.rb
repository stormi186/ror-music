class TracksController < ApplicationController
    ...
    @top_tracks = Favorite.joins('join tracks on favorites.track_id = tracks.id').select('track_id, name, artist_id').group('track_id').order('COUNT(tracks.id) DESC').limit(10)
